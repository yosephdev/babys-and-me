...
import { axkidProducts } from "@/data/axkidProducts";

const AxkidProducts = () => {
  // Select 5 featured products
  const featuredProducts = axkidProducts
    ...
    <section className="py-12 bg-gray-50">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h2 className="text-2xl font-bold">Axkid Säkerhetsprodukter</h2>
            <p className="text-gray-600 mt-2">
              Högsta säkerhet för ditt barn med Axkids prisvinnande bilbarnstolar
            </p>
          </div>
          <div className="hidden md:block">
            <Button variant="outline" asChild>
              <Link to="/produkter?category=Hälsa%20%26%20Säkerhet">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
          {featuredProducts.map(product => (
            <a 
              ...
              onClick={() => {
                console.log(`Product click: ${product.name} from Axkid SE`);
              }}
            >
